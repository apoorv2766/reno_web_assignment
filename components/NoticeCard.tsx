  ...
  return (
    <article className="flex h-full flex-col rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="mb-3 flex items-start justify-between gap-3">
        <h2 className="line-clamp-2 text-lg font-bold text-slate-900">{notice.title}</h2>
        {urgent ? (
      ...
      <p className="mb-4 line-clamp-4 text-sm leading-6 text-slate-700">{notice.body}</p>

      <div className="mt-auto space-y-2 text-sm text-slate-600">
        <p>
          <span className="font-semibold text-slate-800">Category:</span> {formatCategory(notice.category)}
        </p>
        <p>
          <span className="font-semibold text-slate-800">Publish Date:</span> {dateLabel}
        </p>
      </div>
